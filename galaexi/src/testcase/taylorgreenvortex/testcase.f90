...
#if PARABOLIC
USE MOD_LiftingVars,    ONLY: d_gradUx,d_gradUy,d_gradUz
#endif /* PARABOLIC */
#endif /* USE_ACCEL */
IMPLICIT NONE
!----------------------------------------------------------------------------------------------------------------------------------
...
#if PARABOLIC
CALL CopyFromDevice(C_Loc(gradUx),d_gradUx,SIZE_C_DOUBLE, SIZE(gradUx))
CALL CopyFromDevice(C_Loc(gradUy),d_gradUy,SIZE_C_DOUBLE, SIZE(gradUy))
CALL CopyFromDevice(C_Loc(gradUz),d_gradUz,SIZE_C_DOUBLE, SIZE(gradUz))
#endif /* PARABOLIC */
#endif /* USE_ACCEL */
...
DO iElem=1,nElems
  ...
  DO k=0,NAnalyze;DO j=0,NAnalyze;DO i=0,NAnalyze
    !----------------------------------------------------------------------------------------------------------------------------------
    ! 3. Precompute primitive and gradient-based quantities
    ! We don't want to go through getting everything for this over the device, so just
    ! run it on the host, regardless of build.
    CALL ConsToPrim(PP_N,UPrim,U_NAnalyze(:,i,j,k),0,0,0,i,j,k,runOnHost=.TRUE.)
#if PARABOLIC
