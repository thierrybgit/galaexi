...
SUBROUTINE GetPrimitiveStateSurface(U_master,U_slave,UPrim_master,UPrim_slave, &
                                        d_U_master,d_U_slave,d_UPrim_master,d_UPrim_slave,doMPISides,streamID)
! MODULES
USE MOD_Preproc
USE MOD_EOS,      ONLY: ConsToPrim
USE MOD_Mesh_Vars,ONLY: firstInnerSide,lastInnerSide,lastMPISide_YOUR,nSides,firstMPISide_MINE,lastMPISide_MINE
...
INTEGER(C_INT),INTENT(IN)  :: d_U_master,d_U_slave,d_UPrim_master,d_UPrim_slave !< Device keys for arrays
INTEGER,INTENT(IN),OPTIONAL :: streamID                        !< Device stream to run kernel on (USE_ACCEL != OFF only)
LOGICAL,INTENT(IN) :: doMPISides                               !< = .TRUE. only MPISides are filled, =.FALSE. InnerSides 
!----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES
INTEGER          :: mystream
INTEGER          :: firstSideID_master,lastSideID_master
INTEGER          :: firstSideID_slave ,lastSideID_slave
!==================================================================================================================================
...
IF(doMPISides)THEN
  ! fill only MPISides (where the local proc is master)
  firstSideID_master = firstMPISide_MINE
   lastSideID_master =  lastMPISide_MINE ! No MPI YOUR sides for master
  firstSideID_slave  = firstMPISide_MINE
   lastSideID_slave  =  lastMPISide_YOUR
ELSE
  ! fill only InnerSides that do not need communication
  firstSideID_master =              1 ! include BCs for master
   lastSideID_master =  lastInnerSide
  firstSideID_slave  = firstInnerSide ! No BCs for Slave sides
   lastSideID_slave  =  lastInnerSide
END IF

CALL ConsToPrim(PP_N,UPrim_master(:,:,:,firstSideID_master:lastSideID_master),U_master(:,:,:,firstSideID_master:lastSideID_master), &
                  d_UPrim_master,d_U_master,1,firstSideID_master-1,lastSideID_master-firstSideID_master+1, mystream)

CALL ConsToPrim(PP_N,UPrim_slave(:,:,:,firstSideID_slave:lastSideID_slave),U_slave(:,:,:,firstSideID_slave:lastSideID_slave), &
                  d_UPrim_slave, d_U_slave,1,firstSideID_slave-1,lastSideID_slave-firstSideID_slave+1, mystream)
