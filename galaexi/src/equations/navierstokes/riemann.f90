...
INTERFACE Riemann
  MODULE PROCEDURE Riemann_Point_Host
  MODULE PROCEDURE Riemann_Side_Host
END INTERFACE
...
INTERFACE
    SUBROUTINE InitRiemann_Device(riemannOpt, riemannOpt_BC) BIND(C, NAME="InitRiemann_Device")
      USE ISO_C_BINDING
      IMPLICIT NONE
      INTEGER(C_INT), VALUE :: riemannOpt
      INTEGER(C_INT), VALUE :: riemannOpt_BC
    END SUBROUTINE InitRiemann_Device
END INTERFACE
...
!==================================================================================================================================!
SUBROUTINE InitRiemann(riemann_In, riemannBC_In)
! MODULES
USE MOD_Globals
...
! INPUT / OUTPUT VARIABLES
!----------------------------------------------------------------------------------------------------------------------------------
INTEGER, INTENT(IN), OPTIONAL :: riemann_In, riemannBC_In
! LOCAL VARIABLES
INTEGER                 :: Riemann, RiemannBC
!==================================================================================================================================

  ! Set up Riemann solver procedure pointer
  IF (PRESENT(riemann_In)) THEN
    Riemann = riemann_In
  ELSE
  ...
  ! Set up Riemann solver pointer for BC sides
  IF (PRESENT(riemannBC_In)) THEN
    RiemannBC = riemannBC_In
  ELSE
    RiemannBC = GETINTFROMSTR('RiemannBC')
  END IF

  SELECT CASE(RiemannBC)
...
#if (USE_ACCEL != ACCEL_OFF)
  ! If using device acceleration, repeat the func pointer assignment process on the device.
  CALL InitRiemann_Device(Riemann, RiemannBC)
#endif
...
!----------------------------------------------------------------------------------------------------------------------------------
!==================================================================================================================================
!> Computes the numerical flux for a side calling the flux calculation across multiple element sides.
...
!==================================================================================================================================
PPURE SUBROUTINE Riemann_Side_Host(Nloc,FOut,U_L,U_R,UPrim_L,UPrim_R,nv,t1,t2,doBC)
! MODULES
IMPLICIT NONE
!----------------------------------------------------------------------------------------------------------------------------------
! INPUT / OUTPUT VARIABLES
...
!==================================================================================================================================
  ...
  DO j=0,Nloc; DO i=0,Nloc
    ...
    CALL Riemann_Point_Host(FOut(:,i,j),U_L(:,i,j),U_R(:,i,j),UPrim_L(:,i,j),UPrim_R(:,i,j),&
                  nv(:,i,j),t1(:,i,j),t2(:,i,j),doBC)

  END DO; END DO

END SUBROUTINE Riemann_Side_Host
...
!> Attention 2: numerical flux is backrotated at the end of the routine!!
!==================================================================================================================================
PPURE SUBROUTINE Riemann_Point_Host(FOut,U_L,U_R,UPrim_L,UPrim_R,nv,t1,t2,doBC)
! MODULES
...
Fout(MOMV)=nv(:)*F(MOM1)  &
          ...
          +t2(:)*F(MOM3)
Fout(ENER)=F(ENER)
END SUBROUTINE Riemann_Point_Host
