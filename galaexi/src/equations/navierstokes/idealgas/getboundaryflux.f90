...
DO iSide=1,nBCSides
  IF (Boundarytype(BC(iSide),BC_TYPE).EQ.121) THEN
    DO q=0,PP_NZ; DO p=0,PP_N
      ...
      CALL ConsToPrim(PP_N,BCDataPrim(:,p,q,iSide),BCData(:,p,q,iSide),0, 0,0,0,q,p)
...
CASE(2) ! Exact function or refstate
  IF(BCState.EQ.0)THEN
    DO q=0,ZDIM(Nloc); DO p=0,Nloc
      ...
      CALL ConsToPrim(PP_N,UPrim_boundary(:,p,q),Cons,0,0,0,0,q,p)
    END DO; END DO
  ELSE
    DO q=0,ZDIM(Nloc); DO p=0,Nloc
...
DO SideID=1,nBCSides
  ...
  DO q=0,PP_NZ; DO p=0,PP_N
    ...
    CALL ConsToPrim(PP_N,BCDataPrim(:,p,q,SideID),BCData(:,p,q,SideID),0, 0,0,0,q,p)
  END DO; END DO
END DO
