...
ALLOCATE( F(PP_nVar,0:PP_N,0:PP_N,0:PP_N,nElems) )
ALLOCATE( G(PP_nVar,0:PP_N,0:PP_N,0:PP_N,nElems) )
ALLOCATE( H(PP_nVar,0:PP_N,0:PP_N,0:PP_N,nElems) )
...
! If device acceleration, perform allocations on and copies to the device
#if (USE_ACCEL != ACCEL_OFF)
CALL AllocateDeviceMemory(d_D_Hat_T, SIZE_C_DOUBLE, SIZE(D_Hat_T))
CALL CopyToDevice(d_D_Hat_T, C_Loc(D_Hat_T), SIZE_C_DOUBLE, SIZE(D_Hat_T))

#ifdef SPLIT_DG
CALL AllocateDeviceMemory(d_DVolSurf, SIZE_C_DOUBLE, SIZE(DVolSurf))
CALL CopyToDevice(d_DVolSurf, C_Loc(DVolSurf), SIZE_C_DOUBLE, SIZE(DVolSurf))
#endif

CALL AllocateDeviceMemory(d_U, SIZE_C_DOUBLE, SIZE(U))
! Copy for U is done below after we have checked we aren't filling it from restart.
CALL AllocateDeviceMemory(d_U_master, SIZE_C_DOUBLE, SIZE(U_master))
CALL CopyToDevice(d_U_master, C_Loc(U_master), SIZE_C_DOUBLE, SIZE(U_master))
CALL AllocateDeviceMemory(d_U_slave, SIZE_C_DOUBLE, SIZE(U_slave))
CALL CopyToDevice(d_U_slave, C_Loc(U_slave), SIZE_C_DOUBLE, SIZE(U_slave))

#if PP_EntropyVars == 1
CALL AllocateDeviceMemory(d_V, SIZE_C_DOUBLE, SIZE(V))
CALL CopyToDevice(d_V, C_Loc(V), SIZE_C_DOUBLE, SIZE(V))

CALL AllocateDeviceMemory(d_V_master, SIZE_C_DOUBLE, SIZE(V_master))
CALL CopyToDevice(d_V_master, C_Loc(V_master), SIZE_C_DOUBLE, SIZE(V_master))
CALL AllocateDeviceMemory(d_V_slave, SIZE_C_DOUBLE, SIZE(V_slave))
CALL CopyToDevice(d_V_slave, C_Loc(V_slave), SIZE_C_DOUBLE, SIZE(V_slave))
#endif

CALL AllocateDeviceMemory(d_UPrim, SIZE_C_DOUBLE, SIZE(UPrim))
CALL CopyToDevice(d_UPrim, C_Loc(UPrim), SIZE_C_DOUBLE, SIZE(UPrim))
CALL AllocateDeviceMemory(d_UPrim_master, SIZE_C_DOUBLE, SIZE(UPrim_master))
CALL CopyToDevice(d_UPrim_master, C_Loc(UPrim_master), SIZE_C_DOUBLE, SIZE(UPrim_master))
CALL AllocateDeviceMemory(d_UPrim_slave, SIZE_C_DOUBLE, SIZE(UPrim_slave))
CALL CopyToDevice(d_UPrim_slave, C_Loc(UPrim_slave), SIZE_C_DOUBLE, SIZE(UPrim_slave))

CALL AllocateDeviceMemory(d_Flux_master, SIZE_C_DOUBLE, SIZE(Flux_master))
CALL CopyToDevice(d_Flux_master, C_Loc(Flux_master), SIZE_C_DOUBLE, SIZE(Flux_master))
CALL AllocateDeviceMemory(d_Flux_slave,SIZE_C_DOUBLE, SIZE(Flux_slave))
CALL CopyToDevice(d_Flux_slave, C_Loc(Flux_slave), SIZE_C_DOUBLE, SIZE(Flux_slave))

CALL AllocateDeviceMemory(d_Ut,SIZE_C_DOUBLE, SIZE(Ut))
CALL CopyToDevice(d_Ut, C_Loc(Ut), SIZE_C_DOUBLE, SIZE(Ut))
...
! U is now initialized in some way. We can copy it over now.
#if (USE_ACCEL != ACCEL_OFF)
  CALL CopyToDevice(d_U,C_Loc(U), SIZE_C_DOUBLE, SIZE(U))
#endif
...
#endif
#if PP_EntropyVars==1
USE MOD_DG_Vars             ,ONLY: V,V_slave,V_master,d_V, d_V_slave, d_V_master
USE MOD_EOS                 ,ONLY: ConsToEntropy
#endif
! Include anything for the device that is needed regardless of build
! This includes all device keys
USE MOD_Device
USE MOD_DG_Vars             ,ONLY: d_Ut,d_U,d_UPrim,d_U_master,d_U_slave,d_UPrim_master, d_UPrim_slave
USE MOD_DG_Vars             ,ONLY: d_Flux_master, d_Flux_slave
#if (USE_ACCEL != ACCEL_OFF)
USE MOD_DeviceManage        ,ONLY: SynchronizeDevice
USE MOD_DeviceMem
#endif
! IMPLICIT VARIABLE HANDLING
IMPLICIT NONE
...
! 2. Convert Volume solution to primitive
CALL ConsToPrim(PP_N,UPrim,U,d_UPrim,d_U,STREAM_LOW)

! Compute entropy variables
#if PP_EntropyVars == 1
Call ConsToEntropy(PP_N,V,U,d_V,d_U)
#endif
...
#if (FV_ENABLED == 2) && (PP_NodeType==1)
#if PP_EntropyVars == 0
CALL ProlongToFace(PP_nVar,PP_N,U,U_master,U_slave,d_U,d_U_master,d_U_slave,L_Minus,L_Plus,doMPISides=.TRUE.,streamID=STREAM_DEFAULT)
CALL ProlongToFace(PP_N,U,FV_U_master,FV_U_slave,d_U,d_FV_U_master,d_FV_U_slave, &
                                                          L_Minus,L_Plus,doMPISides=.TRUE.,pureFV=.TRUE.,streamID=STREAM_DEFAULT)
#endif /*if PP_EntropyVars == 0*/
#else /*FV_ENABLED*/
#if PP_EntropyVars == 1
CALL ProlongToFace(PP_nVar,PP_N,V,V_master,V_slave,d_V,d_V_master,d_V_slave, &
                                        U_master,U_slave,d_U_master,d_U_slave,L_Minus,L_Plus,doMPISides=.TRUE.)
#else
CALL ProlongToFace(PP_nVar,PP_N,U,U_master,U_slave,d_U,d_U_master,d_U_slave,L_Minus,L_Plus,doMPISides=.TRUE.,streamID=STREAM_MID)
#endif /*if PP_EntropyVars == 1*/
#endif /*FV_ENABLED*/
...
#if (FV_ENABLED == 2) && (PP_NodeType==1)
#if PP_EntropyVars == 0
CALL ProlongToFace(PP_nVar,PP_N,U,U_master,U_slave,d_U,d_U_master,d_U_slave,L_Minus,L_Plus,doMPISides=.FALSE.)
CALL ProlongToFace(PP_nVar,PP_N,U,FV_U_master,FV_U_slave,d_U,d_FV_U_master,d_FV_U_slave, &
                                                      L_Minus,L_Plus,doMPISides=.FALSE.,pureFV=.TRUE.)
#endif /*if PP_EntropyVars == 0*/
#else /*FV_ENABLED*/
#if PP_EntropyVars == 1
CALL ProlongToFace(PP_nVar,PP_N,V,V_master,V_slave,d_V,d_V_master,d_V_slave, &
                                          U_master,U_slave,d_U_master,d_U_slave,L_Minus,L_Plus,doMPISides=.FALSE.)
#else
CALL ProlongToFace(PP_nVar,PP_N,U,U_master,U_slave,d_U,d_U_master,d_U_slave,L_Minus,L_Plus,doMPISides=.FALSE.)
#endif /*if PP_EntropyVars == 1*/
#endif /*FV_ENABLED*/
...
#if USE_MPI
CALL GetPrimitiveStateSurface(U_master,U_slave,UPrim_master,UPrim_slave,d_U_master,d_U_slave,d_UPrim_master, &
                                                                  d_UPrim_slave, doMPISides=.TRUE.,streamID=STREAM_DEFAULT)
#endif
CALL GetPrimitiveStateSurface(U_master,U_slave,UPrim_master,UPrim_slave,d_U_master,d_U_slave,d_UPrim_master, &
                                                                  d_UPrim_slave, doMPISides=.FALSE.,streamID=STREAM_MID)
...
CALL VolInt(Ut,streamID=STREAM_LOW)
...
#else /*((FV_ENABLED == 2) && (PP_NodeType == 1))*/
! 11.5)
CALL SurfIntCons(PP_N,Flux_master,Flux_slave,Ut,.FALSE.,L_HatMinus,L_hatPlus,streamID=STREAM_DEFAULT)
#endif /*((FV_ENABLED == 2) && (PP_NodeType == 1))*/
...
CALL VAXPB_CONST(nTotalU,Ut,d_Ut,-1.0)
...
#if FV_ENABLED
    CALL ApplyJacobian(PP_nVar,PP_N,Ut,d_Ut,toPhysical=.TRUE.,FVE=1)
#endif /*FV_ENABLED*/
  CASE (OVERINTEGRATIONTYPE_CUTOFF)
    CALL Overintegration(Ut)
    CALL ApplyJacobian(PP_nVar,PP_N,Ut,d_Ut,toPhysical=.TRUE.)
  CASE DEFAULT
    CALL ApplyJacobian(PP_nVar,PP_N,Ut,d_Ut,toPhysical=.TRUE.)
END SELECT
