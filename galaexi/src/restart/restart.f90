...
USE MOD_DG_Vars,            ONLY: d_U   ! This is needed for passing to at least ApplyJacobian, regardless of build
! IMPLICIT VARIABLE HANDLING
IMPLICIT NONE
!----------------------------------------------------------------------------------------------------------------------------------
! INPUT/OUTPUT VARIABLES
...
IF (DoRestart) THEN
  ...
  ELSE ! InterpolateSolution
    ...
    IF(N_Restart.GT.PP_N)THEN
      ...
      DEALLOCATE(JNR)
      ! Transform back
      CALL ApplyJacobian(PP_nVar,PP_N,U,d_U,toPhysical=.TRUE.,FVE=0)
    ELSE
      DO iElem=1,nElems
        IF (FV_Elems(iElem).EQ.0) THEN ! DG element
          CALL ChangeBasisVolume(PP_nVar,N_Restart,PP_N,Vdm_NRestart_N,U_local(:,:,:,:,iElem),U(:,:,:,:,iElem))
