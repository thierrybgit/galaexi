...
USE MOD_Mesh_Vars          ,ONLY: sJ,detJac_Ref,Ja_Face,Ja_slave
...
ALLOCATE(Geo(GeoSize,0:PP_N,0:PP_N,firstMPISide_MINE:nSides,0:FV_SIZE))
Geo=0.
...
#if USE_MPI
MPIRequest_Geo=MPI_REQUEST_NULL
CALL StartReceiveMPIData(sJ_slave(:,:,:,:,0),(PP_N+1)*(PP_N+1),1,nSides,MPIRequest_Geo(:,SEND),SendID=2)
CALL ProlongToFace(1,PP_N,detJac_N,sJ_master(:,:,:,:,0),sJ_slave(:,:,:,:,0), &
                                      0,0,0,L_Minus,L_Plus,doMPISides=.TRUE.,pureDG=.TRUE.)
CALL U_Mortar1(sJ_master(:,:,:,:,0),sJ_slave(:,:,:,:,0),doMPISides=.TRUE.,pureDG=.TRUE.)
CALL StartSendMPIData(   sJ_slave(:,:,:,:,0),(PP_N+1)*(PP_N+1),1,nSides,MPIRequest_Geo(:,RECV),SendID=2)
#endif
CALL ProlongToFace(1,PP_N,detJac_N,sJ_master(:,:,:,:,0),sJ_slave(:,:,:,:,0), &
                                      0,0,0,L_Minus,L_Plus,doMPISides=.FALSE.,pureDG=.TRUE.)
...
!==================================================================================================================================
SUBROUTINE CalcSurfMetrics(Nloc,FVE,JaCL_N,XCL_N,Vdm_CLN_N,iElem,NormVec,TangVec1,TangVec2,SurfElem,Face_xGP,Ja_Face,Ja_slave)
! MODULES
...
REAL,INTENT(OUT)   ::    NormVec(3,0:Nloc,0:ZDIM(Nloc),1:nSides,0:FVE) !< (OUT) element face normal vectors
REAL,INTENT(OUT)   ::   TangVec1(3,0:Nloc,0:ZDIM(Nloc),1:nSides,0:FVE) !< (OUT) element face tangential vectors
REAL,INTENT(OUT)   ::   TangVec2(3,0:Nloc,0:ZDIM(Nloc),1:nSides,0:FVE) !< (OUT) element face tangential vectors
REAL,INTENT(OUT)   ::   SurfElem(  0:Nloc,0:ZDIM(Nloc),1:nSides,0:FVE) !< (OUT) element face surface area
...
DO iLocSide=2,5
  ...
  DO dd=1,PP_dim
    ...
    END DO; END DO ! p,q
  END DO ! dd
  IF(PRESENT(Ja_Face)  .AND. flip.EQ.0) Ja_Face(:,:,:,:,SideID)  = Ja_Face_l
  IF(PRESENT(Ja_slave) .AND. flip.NE.0) Ja_slave(:,:,:,:,SideID) = Ja_Face_l
  ...
  CALL SurfMetricsFromJa(Nloc,NormalDir,TangDir,NormalSign,Ja_Face_l,&
                         NormVec(:,:,:,SideID,0),TangVec1(:,:,:,SideID,0),&
                         TangVec2(:,:,:,SideID,0),SurfElem(:,:,SideID,0))
  ...
  IF(MortarType(1,SideID).GT.0)THEN
    ...
    DO iMortar=1,4
      ...
      CALL SurfMetricsFromJa(Nloc,NormalDir,TangDir,NormalSign,Mortar_Ja(:,:,:,:,iMortar),&
                             NormVec(:,:,:,SideID2,0),TangVec1(:,:,:,SideID2,0),&
                             TangVec2(:,:,:,SideID2,0),SurfElem(:,:,SideID2,0))
    END DO
  END IF
END DO
