...
USE MOD_DG_Vars          ,ONLY: U,Ut,nTotalU,d_U,d_Ut
USE MOD_TimeDisc_Vars    ,ONLY: dt,Ut_tmp,RKA,RKb,RKc,nRKStages,CurrentStage
USE MOD_TimeDisc_Vars    ,ONLY: d_Ut_tmp
#if FV_ENABLED
...
DO iStage = 1,nRKStages
  ...
  ! For first stage, just acts as a copy since mRKA=0.0
  CALL VAXPB_2STEP(nTotalU,Ut_tmp,U,Ut,d_Ut_tmp,d_U,d_Ut,mRKA,b_dt(iStage))
...
USE MOD_TimeDisc_Vars    ,ONLY: d_S2,d_UPrev
#if FV_ENABLED
USE MOD_BaseFlow_Vars    ,ONLY: BaseFlowFiltered
USE MOD_Indicator        ,ONLY: CalcIndicator,doIndicatorBaseFlow
#endif /*FV_ENABLED*/
...
DO iStage = 1,nRKStages
  ...
  IF (iStage.EQ.1) THEN
    ! These calls just act as copies as setting Const = 0.0 makes VAXPB_OUT reduce to Out = In.
    CALL VAXPB_OUT(nTotalU,UPrev,U,d_UPrev,d_U,0.0)  !UPrev = U
    CALL VAXPB_OUT(nTotalU,S2,U,d_S2,d_U,0.0)        !S2    = U
  ELSE
    CALL VAXPB_IN(nTotalU,S2,U,d_S2,d_U,RKdelta(iStage))                 !S2    = S2 + U*RKdelta(iStage)
    CALL VAXPB_IN_OUT(nTotalU,U,S2,d_U,d_S2,RKg1(iStage),RKg2(iStage))   !U     = RKg1(iStage)*U + RKg2(iStage)*S2
    CALL VAXPB_IN(nTotalU,U,UPrev,d_U,d_UPrev,RKg3(iStage))              !U     = U + RKg3(ek)*UPrev
  END IF
  CALL VAXPB_IN(nTotalU,U,Ut,d_U,d_Ut,b_dt(iStage))                      !U     = U + Ut*b_dt(iStage)
