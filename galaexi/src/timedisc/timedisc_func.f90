...
USE MOD_DeviceMem
USE MOD_TimeDisc_Vars       ,ONLY: d_Ut_tmp,d_UPrev,d_S2,d_CFLScale
#if PARABOLIC
...
SELECT CASE(TimeDiscType)
  CASE('LSERKW2')
    ALLOCATE(Ut_tmp(1:PP_nVar,0:PP_N,0:PP_N,0:PP_N,1:nElems))
#if (USE_ACCEL != ACCEL_OFF)
    CALL AllocateDeviceMemory(d_Ut_tmp, SIZE_C_DOUBLE, SIZE(Ut_tmp))
#endif
  CASE('LSERKK3')
    ALLOCATE(S2   (1:PP_nVar,0:PP_N,0:PP_N,0:PP_NZ,1:nElems) &
            ,UPrev(1:PP_nVar,0:PP_N,0:PP_N,0:PP_NZ,1:nElems))
#if (USE_ACCEL != ACCEL_OFF)
    CALL AllocateDeviceMemory(d_S2, SIZE_C_DOUBLE, SIZE(S2))
    CALL AllocateDeviceMemory(d_UPrev, SIZE_C_DOUBLE, SIZE(UPrev))
#endif
  CASE('ESDIRK')
...
#if (USE_ACCEL != ACCEL_OFF)
  ! This is the main copy back if ouput is to be performed
  CALL CopyFromDevice(C_Loc(U),d_U,SIZE_C_DOUBLE, SIZE(U))
#endif /* USE_ACCEL */
END IF
...
! LOCAL VARIABLES
!==================================================================================================================================
CALL CalcTimeStep(dt_Min,errType)
...
#if (USE_ACCEL != ACCEL_OFF)
CALL CopyToDevice(d_CFLScale, C_Loc(CFLScale), SIZE_C_DOUBLE, SIZE(CFLScale))
#if PARABOLIC
CALL CopyToDevice(d_DFLScale, C_Loc(DFLScale), SIZE_C_DOUBLE, SIZE(DFLScale))
#endif
#endif /* USE_ACCEL */
