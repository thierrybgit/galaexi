...
# GPU libraries
# =========================================================================
SET(LIBS_USE_ACCEL "OFF" CACHE STRING "Build with support for accelerator devices (off/nvidia/amd).")
SET_PROPERTY(CACHE LIBS_USE_ACCEL PROPERTY STRINGS OFF NVIDIA AMD)
IF(NOT (LIBS_USE_ACCEL STREQUAL "OFF"))
  IF (LIBS_USE_ACCEL STREQUAL "NVIDIA")
    IF (CMAKE_Fortran_COMPILER_ID MATCHES "NVHPC")
      ADD_DEFINITIONS(-DUSE_ACCEL=1)
      ...
      ENDIF()
    ELSE()
      MESSAGE(FATAL_ERROR "GALAEXI currently only supports the NVHPC compiler when building with NVIDIA devices")
    ENDIF()
  ELSEIF(LIBS_USE_ACCEL STREQUAL "AMD")
    IF(${CMAKE_VERSION} VERSION_LESS "3.21.0")
      MESSAGE(FATAL_ERROR "Must use CMAKE version 3.21.0 or newer to compile GALAEXI with AMD offloading.")
    ENDIF()
    IF ((CMAKE_Fortran_COMPILER_ID MATCHES "Flang") OR (CMAKE_Fortran_COMPILER_ID MATCHES "Cray"))
      ADD_DEFINITIONS(-DUSE_ACCEL=2)
      FIND_PACKAGE(hip REQUIRED)
      LINK_LIBRARIES(hip::device)
      ENABLE_LANGUAGE(HIP)
      SET (CMAKE_HIP_FLAGS "${CMAKE_HIP_FLAGS} -std=c++17 -fPIC -mno-unsafe-fp-atomics")
    ELSE()
      MESSAGE(FATAL_ERROR "GALAEXI currently only supports the Cray or AMD LLVM compilers when building for AMD devices.")
    ENDIF()
  ENDIF()

  # Handle hybrid (APU) chips
  OPTION(USE_APU_ARCH "Enable device management for hybrid (APU) architectures" OFF)
  IF (USE_APU_ARCH)
    ADD_DEFINITIONS(-DUSE_HYBRID)
  ENDIF()
ELSE()
  ADD_DEFINITIONS(-DUSE_ACCEL=0)
ENDIF()
